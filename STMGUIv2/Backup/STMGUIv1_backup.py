...
def addBtnCmd():
	...
	if modeOption == 1:
		...
		result = STMClass.addTransition(entry1.get(), entry2.get(), entry3.get(), entry4.get())
		if result == "Success":
...
def updateBtnCmd():
	if modeOption == 1:
		...
		result = STMClass.updateTransition(entry1.get(), entry2.get(), entry3.get(), entry4.get())
		if result == "Success":
...
def findBtnCmd():
	if modeOption == 1:
		result = STMClass.findState(entry1.get())
		popupmsg(result)
	elif modeOption == 2:
		result = STMClass.findTransition(entry3.get(), entry4.get())
		popupmsg(result)
...
def readCoordinates():
	...
	for st, co in zip(STMClass.mySTM.statesDict, coordinates):
		stateCoordinates[STMClass.mySTM.statesDict[st].name] = co
...
def updatePyWindow():
	...
	for tr in STMClass.mySTM.transitionsDict:
		srcx, srcy = stateCoordinates[STMClass.mySTM.transitionsDict[tr].src.name].split("x")
		destx, desty = stateCoordinates[STMClass.mySTM.transitionsDict[tr].dest.name].split("x")
...
def paintAllTransitions(color):
	...
	transitionColor = {x: color for x in STMClass.mySTM.transitionsDict}

def paintAllStates(color):
	global stateColor
	stateColor = {x: color for x in STMClass.mySTM.statesDict}
...
def updateInputSchedule():
	...
	for i in range(1, 12):
		for datarow in data:
			inputUpdateSchedule.enter(float(header[i]), 1, STMClass.updateInput, argument=(datarow[0], datarow[i]))
	file.close()
...
def animationTryStep():
	...
	if animationRun == 1:
		if animationSteps > 0:
			nextstate = STMClass.getNextStateFrom(currAnimationState)
			animationSteps -= 1
			if nextstate != None:
